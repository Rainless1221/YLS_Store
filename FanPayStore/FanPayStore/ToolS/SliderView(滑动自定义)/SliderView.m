...
@property (nonatomic, strong) UIView *slider;//滑动的按钮视图
...
- (void)valueChangeWithFloat:(CGFloat)num{
    ...
    if ([self.delegate respondsToSelector:@selector(starEvaluator:currentValue:)]) {
        [self.delegate starEvaluator:self currentValue:value];
    }
}
...
- (UILabel *)valueLabel{
    if (!_valueLabel) {
        ...
        _valueLabel.textColor = [UIColor colorWithRed:61/255.f green:138/255.f blue:255/255.f alpha:1.0];
...
- (NSArray *)trackColors{
    if (!_trackColors || _trackColors.count < 2) {
        _trackColors = @[(__bridge id)[UIColor colorWithRed:227/255.f green:238/255.f blue:255/255.f alpha:1.0].CGColor, (__bridge id)[UIColor colorWithRed:227/255.f green:238/255.f blue:255/255.f alpha:1.0].CGColor];
    }
    return _trackColors;
}
...
- (UIColor *)normalColor{
    if (_normalColor == nil) {
        _normalColor = [UIColor colorWithRed:227/255.f green:238/255.f blue:255/255.f alpha:1.0];
    }
    return _normalColor;
}
