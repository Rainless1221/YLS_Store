...
-(void)setupUI
{
    ...
    self.dateView=[[UIView alloc] initWithFrame:CGRectMake(W(30), W(172), ScreenWidth-W(60), H(550))];
...
#pragma mark - 当选的横行
    UIView *line1=[[UIView alloc] initWithFrame:CGRectMake(10, self.dateView.frame.size.height/2-15/2.0, 160, 1)];
    line1.backgroundColor=RGBA(61, 138, 255,  1.0);
//    [self.dateView addSubview:line1];
    
    UIView *line=[[UIView alloc] initWithFrame:CGRectMake(10, self.dateView.frame.size.height/2-15/2.0+40, 160, 1)];
    line.backgroundColor=RGBA(61, 138, 255,  1.0);
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    ...
    if (timeHidden == YES) {
        numberArr = @[@(yearNum),@(monthNum),@(dayNum),@(hourNum),@(minuteNUm)];
    }else{
        numberArr = @[@(yearNum),@(monthNum),@(dayNum)];
    }
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    ((UILabel *)[pickerView.subviews objectAtIndex:2]).backgroundColor = RGBA(61, 138, 255,  1.0);//[UIColor clearColor];
