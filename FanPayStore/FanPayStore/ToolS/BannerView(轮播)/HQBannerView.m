...
#pragma mark -  页码位置
    switch (pageTextAliment) {
        ...
        case HQBannerViewPageTextAlimentRightTop: {
            _bannerPageText = [[HQBannerPageText alloc] initWithFrame:CGRectMake(self.frame.size.width - 15-width, 15, width, height)];
            _bannerPageText.PageText = [NSString stringWithFormat:@"1/%ld",(unsigned long)self.imageURLArray.count];
            [self addSubview:_bannerPageText];
        }
            break;
