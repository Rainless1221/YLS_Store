...
- (void)show:(NSArray *)dataArr and:(NSString *)title andTitleColor:(UIColor *)titleColor colorArr:(NSArray *)color {
    ...
    for (int i = 0; i < dataArr.count; i++) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            float titleWidth= 0;
            titleWidth = [LRSLinesPaoPaoView measureSinglelineStringWidth:dataTitle andFont:[UIFont systemFontOfSize:13]];
    ...
    if (title && title.length > 0) {
        ...
        titleLab.textColor = [UIColor redColor];//[self colorWithHexString:@"#d3d3d3"];
        titleLab.text = title;
...
+(CGSize)getWidthAndHeight:(NSArray *)dataArray{
    ...
    for (int i = 0; i < dataArray.count; i++) {
        id obj = dataArray[i];
        if ([obj isKindOfClass:[NSString class]]) {
            float sizeWidth = [LRSLinesPaoPaoView measureSinglelineStringWidth:(NSString *)obj andFont:[UIFont systemFontOfSize:13]];
