...
- (instancetype)init{
    ...
    if (self) {
        ...
        UILongPressGestureRecognizer *Gesture1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(GestureGestureRecognized:)];
        [self addGestureRecognizer:Gesture1];
    }
    return self;
}

-(void)GestureGestureRecognized:(id)sender{
    ...
    if ([self.dataSource respondsToSelector:@selector(Gesture:)]) {
        [self.dataSource Gesture:_originalIndexPath];
    }
}
