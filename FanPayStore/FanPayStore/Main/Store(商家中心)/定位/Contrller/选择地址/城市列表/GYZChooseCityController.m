...
@interface GYZChooseCityController ()<GYZCityGroupCellDelegate,UISearchBarDelegate,CLLocationManagerDelegate,MAMapViewDelegate,AMapSearchDelegate>
...
//    [self.navigationItem setLeftBarButtonItem:cancelBarButton];
    ...
    self.searchBar = [[UISearchBar alloc]initWithFrame:CGRectMake(0, 0, ScreenW, 44.0f)];
//    self.searchBar.barStyle     = UIBarStyleDefault;
    ...
    [self.tableView setSectionIndexColor:[UIColor colorWithRed:61/255.0 green:138/255.0 blue:255/255.0 alpha:1.0]];
    ...
    [self.locationManager1 requestLocationWithReGeocode:YES completionBlock:^(CLLocation *location, AMapLocationReGeocode *regeocode, NSError *error) {
        ...
        //获取城市
        NSString *currCity = regeocode.city;
        if (!currCity) {
            //四大直辖市的城市信息无法通过locality获得，只能通过获取省份的方法来获得（如果city为空，则可知为直辖市）
            currCity = regeocode.province;
        }
...
//                city.cityID = [dic objectForKey:@"city_key"];
//                city.cityName = [dic objectForKey:@"city"];
//                city.shortName = [dic objectForKey:@"short_name"];
//                city.pinyin = [dic objectForKey:@"pinyin"];
//                city.initials = [dic objectForKey:@"initials"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(0, cell.bottom-1, ScreenW, 1)];
...
//            }
            ...
            if (chinese.location != NSNotFound || letters.location != NSNotFound) {
                [self.searchCities addObject:city];
            }
...
#pragma mark - GET
- (AMapLocationManager *)locationManager1 {
    if (!_locationManager1) {
        _locationManager1 = [[AMapLocationManager alloc]init];
        [_locationManager1 setDesiredAccuracy:kCLLocationAccuracyHundredMeters];
        _locationManager1.locationTimeout = 2;
        _locationManager1.reGeocodeTimeout = 2;
    }
    return _locationManager1;
}
@end
