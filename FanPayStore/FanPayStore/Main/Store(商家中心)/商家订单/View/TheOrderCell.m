...
-(void)uiConfigure{
    self.BaseView = [[UIView alloc]initWithFrame:CGRectMake(15, 15, ScreenW - 30, 350)];
...
- (void)setData:(NSDictionary *)Data{
    ...
    if (goodMen>3) {
        goodMen = 3;
        self.BaseView.height = 350+goodMen*30;
    }else{
        self.detailsButton.hidden = YES;
        self.detailslabel.hidden = YES;
        self.BaseView.height = 300+goodMen*30;
    }
...
-(void)DeleteAction{
    ...
    if (self.delagate && [self.delagate respondsToSelector:@selector(delete_order2:)]) {
        [self.delagate delete_order2:self.Data];
    }
}
-(void)DayinAction{
    
    if (self.delagate && [self.delagate respondsToSelector:@selector(printe2:)]) {
        [self.delagate printe2:self.Data];
    }
