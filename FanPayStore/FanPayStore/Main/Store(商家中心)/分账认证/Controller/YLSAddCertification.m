...
#import "YLSAddCertification.h"

@interface YLSAddCertification ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation YLSAddCertification

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"注册个人分账账户";
...
-(void)createUI{
    ...
    /*填写部分*/
    NSArray *array = @[@"银行卡号",@"所属银行",@"身份证号",@"预留手机",@"卡主姓名"];
    for (int i=0; i<5; i++) {
...
//    VC.Navtitle = @"短信认证";
//    VC.agreeUrl = @"http://103.27.7.20/h5/other/back.html";
...
//    return ;
    ...
    NSDictionary *Dict = @{@"cust_name":[NSString stringWithFormat:@"%@",self.Name.text],//姓名
                           @"id_no":[NSString stringWithFormat:@"%@",self.IDcard_num.text],//身份证号
                           @"mobile_no":[NSString stringWithFormat:@"%@",self.mobile.text],//银行卡预留手机号
                           @"bank_code":[NSString stringWithFormat:@"%@",self.bank_code],//所属银行行号
                           @"card_no":[NSString stringWithFormat:@"%@",number],//银行卡号
                           };
    YBWeakSelf
    [[FBHAppViewModel shareViewModel]  exec_register_non_enterprise_account:model.merchant_id andstore_id:model.store_id andjoinDict:Dict Success:^(NSDictionary *resDic) {
        if ([resDic[@"status"] integerValue]==1) {
            NSDictionary *DIC = resDic[@"data"];
...
-(UITextField *)card_number{
    if (!_card_number) {
        ...
        _card_number.placeholder = @"请输入银行卡号";
...
-(UITextField *)IDcard_num{
    if (!_IDcard_num) {
        ...
        _IDcard_num.placeholder = @"请输入卡主身份证号";
...
-(UITextField *)mobile{
    if (!_mobile) {
        ...
        _mobile.placeholder = @"请输入卡主预留手机号";
...
-(UIButton *)bank_name{
    if (!_bank_name) {
        ...
        [_bank_name setTitle:@"请选择银行卡所属银行" forState:UIControlStateNormal] ;
